...
#    define CHECKCHAR_MOVEBUF_SPACE_DECSIZE                           \
        do {                                                          \
            ...
            if (ret1 < 1) return ZCE_ERROR_SYNTAX;                    \
        } while (0)
...
static inline int pack_builtin(zce_byte* buf, int size, T val) {
    if (buf && size < (int)sizeof(T)) return ZCE_ERROR_SHRTLEN;
...
static inline int unpack_builtin(T* val, const zce_byte* buf, int size) {
    if (size < (int)sizeof(T)) return ZCE_ERROR_SHRTLEN;
...
template <typename T>
static inline int pack_builtin_array(zce_byte* buf, zce_int32 size, const std::vector<T>& val,
                                     zce_int32 fixed_len) {
    int len = 0, ret = 0, i = 0;
    zce_int32 alen = (zce_int32)val.size();

    if (fixed_len > 0 && fixed_len != alen) {
        ZCE_DEBUG((ZLOG_DEBUG, "pack_builtin_array array=%s, len=%u, fixed_len=%u",
...
template <typename T>
static inline int unpack_builtin_array(std::vector<T>& val, const zce_byte* buf, zce_int32 size,
                                       zce_int32 fixed_len) {
    int len = 0, ret = 0, i = 0;
    zce_int32 alen = 0;
    ...
    if (fixed_len == 0) {
        ...
        if (alen != fixed_len && fixed_len != 0) {
            ZCE_DEBUG((ZLOG_DEBUG, "unpack_builtin_array array=%s, len=%u, fixed_len=%u\n",
                       typeid(T).name(), alen, fixed_len));
            return ZCE_ERROR_SYNTAX;
        }
    } else {
...
static inline int unpack_builtin_fix_array(P& val, const zce_byte* buf, zce_int32 size,
                                           zce_int32 fixed_len, bool alwasy_has_array_len) {
    int len = 0, ret = 0, i = 0;
    zce_int32 alen = 0;
...
template <typename T>
static inline int pack_rawptr_pair(zce_byte* buf, zce_int32 size, const std::pair<T*, T*>& val,
                                   zce_int32 fixed_len) {
    int len = 0, ret = 0;
    zce_int32 alen = (zce_int32)(val.second - val.first);

    if (fixed_len > 0 && fixed_len != alen) {
        ZCE_DEBUG((ZLOG_DEBUG, "[proto_pack] array=%s, len=%u, fixed_len=%u\n", typeid(T).name(),
                   alen, fixed_len));
        return ZCE_ERROR_SYNTAX;
    }
...
template <typename T>
static inline int unpack_rawptr_pair(std::pair<T*, T*>& val, const zce_byte* buf, zce_int32 size,
                                     zce_int32 fixed_len) {
    int len = 0, ret = 0;
    zce_int32 alen = 0;
...
int ZCE_API pack_builtin_string(zce_byte* buf, zce_int32 size, const char* data, zce_int32 len,
                                zce_int32 fixed_len, zce_byte version);
int ZCE_API pack_builtin_string(zce_byte* buf, zce_int32 size, const std::string& val,
                                zce_int32 fixed_len, zce_byte version);
int ZCE_API unpack_builtin_string(std::string& val, const zce_byte* buf, zce_int32 size,
                                  zce_int32 fixed_len, zce_byte version);
...
template <typename T>
int pack_ie_array(zce_byte* buf, zce_int32 size, const std::vector<T>& val, int /*fixed_len*/,
                  unsigned char version) {
    int len = 0, ret = 0;
    for (auto iter = val.begin(); iter != val.end(); ++iter) {
        len = zdp_pack(*iter, buf, size, version);
...
template <typename T>
int unpack_ie_array(std::vector<T>& val, const zce_byte* buf, zce_int32 size, int /*fixed_len*/,
                    unsigned char version) {
    int len = 0, ret = 0;
    do {
        T tempval;
...
zce_int32 ie_array_total_length(const std::vector<T>& val, unsigned char version) {
    ...
    for (iter = val.begin(); iter != val.end(); ++iter) len += zdp_total_length(*iter, version);
    return len;
}
