...
inline bool parse_bool(const std::string& _value) {
    if (_value == "True" || _value == "true" || _value == "TRUE" || _value == "1") return true;
    return false;
}

inline char parse_int8(const std::string& _value) { return (char)parse_value<char>(_value); }

inline unsigned char parse_uint8(const std::string& _value) {
    return (unsigned char)parse_value<unsigned char>(_value);
}

inline short parse_int16(const std::string& _value) { return parse_value<short>(_value); }

inline unsigned short parse_uint16(const std::string& _value) {
    return parse_value<unsigned short>(_value);
}

inline int parse_int32(const std::string& _value) { return parse_value<int>(_value); }

inline unsigned int parse_uint32(const std::string& _value) {
    return parse_value<unsigned int>(_value);
}

inline size_t parse_sizet(const std::string& _value) { return parse_value<size_t>(_value); }

inline float parse_float(const std::string& _value) { return parse_value<float>(_value); }

inline double parse_double(const std::string& _value) { return parse_value<double>(_value); }
...
class StringSplitter {
  public:
    ...
    typedef typename std::basic_stringstream<T, std::char_traits<T>, std::allocator<T>>
        tstringstream;
    /** @param big the string to be split
    @param splitter the delimiter
    */
    StringSplitter(const T* big, const T* splitter) : _big(big), _splitter(splitter) {}

    /** @return true if more to be taken via next() */
    bool more() { return _big[0]; }
...
template <typename T>
inline void split(std::vector<typename StringSplitter<T>::tstring>& _ret, const T* _source,
                  const T* _delims) {
...
inline std::vector<std::wstring> split(const std::wstring& _source,
                                       const wchar_t* _delims = L"\t\n ") {
    return split(_source.c_str(), _delims);
}
...
inline bool start_with(const std::string& _source, const std::string& _value) {
    ...
    if (_source.size() < count) return false;
    for (size_t index = 0; index < count; ++index) {
        if (_source[index] != _value[index]) return false;
    }
    return true;
}
...
inline bool end_with(const std::string& _source, const std::string& _value) {
    ...
    for (size_t index = 0; index < count; ++index) {
        if (_source[index + offset] != _value[index]) return false;
    }
    return true;
}
...
inline std::string_view remove_utf8_bom(std::string_view sv) {
    constexpr unsigned char utf8_bom[] = {0xEF, 0xBB, 0xBF};
    if (sv.size() >= 3 && static_cast<unsigned char>(sv[0]) == utf8_bom[0] &&
        static_cast<unsigned char>(sv[1]) == utf8_bom[1] &&
        static_cast<unsigned char>(sv[2]) == utf8_bom[2]) {
        return sv.substr(3);
    }
    return sv;
}
