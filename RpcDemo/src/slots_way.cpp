...
static double _calc_rtp_n(const dbl_vector& p_vec, const dbl_vector& notp_vec, const int_vector& peifu_vec, int pengcnt)
{
    ...
    double rtp = 0;
    for (int i = 0; i < _row_num[0]; ++i) {
        for (int j = 0; j < _row_num[1]; ++j) {
            for (int k = 0; k < _row_num[2]; ++k) {
                if (pengcnt == 3) {
                    rtp += NCR(_row_num[0], i + 1) * pow(p_vec[0], i + 1) * i
                        * NCR(_row_num[1], j + 1) * pow(p_vec[1], j + 1) * j
                        * NCR(_row_num[2], k + 1) * pow(p_vec[2], k + 1) * k;
                }
                else {
                    for (int l = 0; l < _row_num[3]; ++l) {
                        if (pengcnt == 4) {
                            rtp += NCR(_row_num[0], i + 1) * pow(p_vec[0], i + 1) * i
                                * NCR(_row_num[1], j + 1) * pow(p_vec[1], j + 1) * j
                                * NCR(_row_num[2], k + 1) * pow(p_vec[2], k + 1) * k
                                * NCR(_row_num[3], l + 1) * pow(p_vec[3], l + 1) * l;
                        }
                        else {
                            for (int m = 0; m < _row_num[4]; ++m) {
                                if (pengcnt == 5) {
                                    rtp += NCR(_row_num[0], i + 1) * pow(p_vec[0], i + 1) * i
                                        * NCR(_row_num[1], j + 1) * pow(p_vec[1], j + 1) * j
                                        * NCR(_row_num[2], k + 1) * pow(p_vec[2], k + 1) * k
                                        * NCR(_row_num[3], l + 1) * pow(p_vec[3], l + 1) * l
                                        * NCR(_row_num[4], m + 1) * pow(p_vec[4], m + 1) * m;
                                }
                                else {
                                    for (int n = 0; n < _row_num[5]; ++n) {
                                        rtp += NCR(_row_num[0], i + 1) * pow(p_vec[0], i + 1) * i
                                            * NCR(_row_num[1], j + 1) * pow(p_vec[1], j + 1) * j
                                            * NCR(_row_num[2], k + 1) * pow(p_vec[2], k + 1) * k
                                            * NCR(_row_num[3], l + 1) * pow(p_vec[3], l + 1) * l
                                            * NCR(_row_num[4], m + 1) * pow(p_vec[4], m + 1) * m
                                            * NCR(_row_num[5], n + 1) * pow(p_vec[5], n + 1) * n;
                                    }
                                }
                            }
                        }
...
void calc_rtp()
{
    dbl_matrix p_matrix = _calc_p_matrix(_relative_expect_matrix);
    std::cout << "---------p---------" << std::endl;
    print_vec(p_matrix);
