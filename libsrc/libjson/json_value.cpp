...
static const std::regex isDoubleRegex("^[+-]?([0-9]*[.])?[0-9]+([eE][+-]?[0-9]+)?$");
...
double Value::asDouble() const {
    switch (type_) {
        ...
        case stringValue:
            ...
            if (!value_.string_ || !*value_.string_) return 0.0;
            // regex to check if the string is a valid double
            if (!std::regex_match(value_.string_, isDoubleRegex)) break;
            // Use atof() to convert the string to double.
